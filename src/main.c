 ...
 *
 * @details 
 * This file contains some test and benchmarking functions.
 *******************************************************************************
 */
...
// Assembler function to return the Time-Stamp Counter (TSC) of CPU (see file
// rdtsc64.S for an implementation to be used with GCC/Clang and rdtsc64.asm 
// for an implementation for Microsoft Visual Studio) 
...
void test_ecdh()
{
  ...
  uint32_t sk_c[8] = { mpi29_rand32(), mpi29_rand32(), mpi29_rand32(), \
    mpi29_rand32(), mpi29_rand32(), mpi29_rand32(), mpi29_rand32(),    \
    mpi29_rand32() };
  // Dave's private key (random)
  uint32_t sk_d[8] = { mpi29_rand32(), mpi29_rand32(), mpi29_rand32(), \
    mpi29_rand32(), mpi29_rand32(), mpi29_rand32(), mpi29_rand32(),    \
    mpi29_rand32() };
  ...
  puts("\n* Public key:");
  mpi29_conv_29to32(r, NWORDS, pk_a, NWORDS);
  mpi29_print("  - Alice : ", r, 8);
  mpi29_conv_29to32(r, NWORDS, pk_b, NWORDS);
  mpi29_print("  - Bob   : ", r, 8);
  mpi29_conv_29to32(r, NWORDS, pk_c, NWORDS);
  mpi29_print("  - Carol : ", r, 8);
  mpi29_conv_29to32(r, NWORDS, pk_d, NWORDS);
  ...
  mpi29_conv_29to32(r, NWORDS, ss_a, NWORDS);
  mpi29_print("  - Alice : ", r, 8);

  // 2nd instance 
  for (i = 0; i < NWORDS; i++) ss_b[i] = VEXTR32(ss[i], 2);
  mpi29_conv_29to32(r, NWORDS, ss_b, NWORDS);
  mpi29_print("  - Bob   : ", r, 8);

  // 3rd instance 
  for (i = 0; i < NWORDS; i++) ss_c[i] = VEXTR32(ss[i], 4);
  mpi29_conv_29to32(r, NWORDS, ss_c, NWORDS);
  mpi29_print("  - Carol : ", r, 8);

  // 4th instance 
  for (i = 0; i < NWORDS; i++) ss_d[i] = VEXTR32(ss[i], 6);
  mpi29_conv_29to32(r, NWORDS, ss_d, NWORDS);
  mpi29_print("  - Dave  : ", r, 8);
